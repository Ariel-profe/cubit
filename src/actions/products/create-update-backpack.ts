...
const backpackSchema = z.object({
    ...
    compartments: z.coerce
        .number()
        .min(0)
        .transform(val => Number(val.toFixed(0))),
    lateralPockets: z.coerce
        .number()
        .min(0)
        .transform(val => Number(val.toFixed(0))),
    superiorPockets: z.coerce
        .number()
        .min(0)
        .transform(val => Number(val.toFixed(0))),
    frontPockets: z.coerce
        .number()
        .min(0)
        .transform(val => Number(val.toFixed(0))),
    notebookCompartment: z.string(),
    notebookSize: z.string(),
});

export const createUpdateBackpack = async (formData: FormData) => {
    const data = Object.fromEntries(formData);

    let productParsed = backpackSchema.safeParse(data);
    ...
    try {
        const prismaTx = await prisma.$transaction(async (tx) => {
            ...
            const materialsArray = rest.materials ? rest.materials.split(',').map((materials: string) => materials.trim().toLowerCase()) : [];

            if (id) {
                // Update product
                product = await prisma.backpack.update({
                    where: { id },
                    data: {
                        ...rest,
                        tags: {
                ...
                // Create product
                product = await prisma.backpack.create({
                    data: {
                        ...rest,
                        tags: {
                            set: tagsArray
            ...
            if (formData.getAll("images")) {
                ...
                await prisma.productImage.createMany({
                    data: images.map(image => ({
                        ...
                        backpackId: prisma.backpack ? product.id : null
                    }))
                })
            };
        ...
        // Revalidate path
        revalidatePath('/admin/productos/mochilas');
        revalidatePath(`/admin/productos/mochilas/${product.slug}`);
        revalidatePath(`/productos/mochilas/${product.slug}`);
