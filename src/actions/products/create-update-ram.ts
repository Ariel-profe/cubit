...
import { Ram, Prisma } from "@prisma/client";
...
const ramSchema = z.object({
    ...
    capacity: z.string().min(3).max(100),
    speed: z.string().min(3).max(100),
});

export const createUpdateRam = async (formData: FormData) => {
    ...
    let productParsed = ramSchema.safeParse(data);
    ...
    try {
        const prismaTx = await prisma.$transaction(async (tx) => {
            ...
            if (id) {
                // Update product
                product = await prisma.ram.update({
                    where: { id },
                    data: {
                        ...rest,
                        tags: {
                ...
                // Create product
                product = await prisma.ram.create({
                    data: {
                        ...rest,
                        tags: {
                            set: tagsArray
            ...
            if (formData.getAll("images")) {
                ...
                await prisma.productImage.createMany({
                    data: images.map(image => ({
                        url: image!,
                        ramId: prisma.ram ? product.id : null
                    }))
                })
            };
        ...
        // Revalidate path
        revalidatePath('/admin/productos/rams');
        revalidatePath(`/admin/productos/rams/${product.slug}`);
        revalidatePath(`/productos/rams/${product.slug}`);
