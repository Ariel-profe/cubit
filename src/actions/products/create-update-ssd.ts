...
import { Ssd, Prisma } from "@prisma/client";
...
export const createUpdateSsd = async (formData: FormData) => {
    const data = Object.fromEntries(formData);

    let productParsed = ssdSchema.safeParse(data);
    ...
    try {
        const prismaTx = await prisma.$transaction(async (tx) => {
            ...
            if (id) {
                // Update product
                product = await prisma.ssd.update({
                    where: { id },
                    data: {
                        ...rest,
                        tags: {
                ...
                // Create product
                product = await prisma.ssd.create({
                    data: {
                        ...rest,
                        tags: {
                            set: tagsArray
            ...
            if (formData.getAll("images")) {
                ...
                await prisma.productImage.createMany({
                    data: images.map(image => ({
                        url: image!,
                        ssdId: prisma.ssd ? product.id : null
                    }))
                })
            };
        ...
        // Revalidate path
        revalidatePath('/admin/productos/discos-solidos');
        revalidatePath(`/admin/productos/discos-solidos/${product.slug}`);
        revalidatePath(`/productos/discos-solidos/${product.slug}`);
