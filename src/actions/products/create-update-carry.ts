...
export const createUpdateCarry = async (formData: FormData) => {
    ...
    try {
        const prismaTx = await prisma.$transaction(async (tx) => {
            ...
            if (id) {
                // Update product
                product = await prisma.carry.update({
                    where: { id },
                    data: {
                        ...rest,
                        tags: {
                ...
                // Create product
                product = await prisma.carry.create({
                    data: {
                        ...rest,
                        tags: {
                            set: tagsArray
            ...
            if (formData.getAll("images")) {
                ...
                await prisma.productImage.createMany({
                    data: images.map(image => ({
                        ...
                        carryId: prisma.carry ? product.id : null
                    }))
                })
            };
        ...
        revalidatePath(`/admin/productos/carrys/${product.slug}`);
        revalidatePath(`/productos/carrys/${product.slug}`);
