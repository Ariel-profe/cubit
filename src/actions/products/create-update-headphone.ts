...
const headphoneSchema = z.object({
    ...
    sensitivity: z.string().min(3).max(100),
    type: z.string().min(3).max(100),
    connectionType: z.string().min(3).max(100),
    frequencyRange: z.string().min(3).max(100),
    colors: z.string(),
    microphone: z.string().min(1).max(10),
    ledLighting: z.string().min(1).max(10),
    gamer: z.string().min(1).max(10),
});

export const createUpdateHeadphone = async (formData: FormData) => {
    const data = Object.fromEntries(formData);

    let productParsed = headphoneSchema.safeParse(data);
    ...
    try {
        const prismaTx = await prisma.$transaction(async (tx) => {
            ...
            const colorsArray = rest.colors ? rest.colors.split(',').map((materials: string) => materials.trim().toLowerCase()) : [];

            if (id) {
                // Update product
                product = await prisma.headphone.update({
                    where: { id },
                    data: {
                        ...rest,
                        tags: {
                ...
                // Create product
                product = await prisma.headphone.create({
                    data: {
                        ...rest,
                        tags: {
                            set: tagsArray
            ...
            if (formData.getAll("images")) {
                ...
                await prisma.productImage.createMany({
                    data: images.map(image => ({
                        ...
                        headphoneId: prisma.headphone ? product.id : null
                    }))
                })
            };
        ...
        // Revalidate path
        revalidatePath('/admin/productos/auriculares');
        revalidatePath(`/admin/productos/auriculares/${product.slug}`);
        revalidatePath(`/productos/auriculares/${product.slug}`);
