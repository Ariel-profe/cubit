...
import { Coolerpad, Prisma } from "@prisma/client";

// Configure Cloudinary
cloudinary.config(process.env.CLOUDINARY_URL ?? '');

const coolerpadSchema = z.object({
...
export const createUpdateCoolerpad = async (formData: FormData) => {
    const data = Object.fromEntries(formData);

    let productParsed = coolerpadSchema.safeParse(data);
    ...
    try {
        const prismaTx = await prisma.$transaction(async (tx) => {
            ...
            if (id) {
                ...
                // Create product
                product = await prisma.coolerpad.create({
                    data: {
                        ...rest,
                        tags: {
                            set: tagsArray
            ...
            if (formData.getAll("images")) {
                ...
                await prisma.productImage.createMany({
                    data: images.map(image => ({
                        ...
                        coolerpadId: prisma.coolerpad ? product.id : null
                    }))
                })
            };
        ...
        // Revalidate path
        revalidatePath('/admin/productos/cooler-pads');
        revalidatePath(`/admin/productos/cooler-pads/${product.slug}`);
        revalidatePath(`/productos/cooler-pads/${product.slug}`);
