...
export default async function HddAdminPage({ searchParams }: Props) {

  const page = (await searchParams).page ? parseInt((await searchParams).page!) : 1;

  const { products } = await getPaginatedProductsWithImages({ page, model: "hdd" });
  ...
  return (
    <section className="flex flex-col container mx-auto px-3 mt-10 lg:mt-20 overflow-hidden">
      ...
      <div className='mt-5 flex justify-end'>
        <Button>
          <Link href="/admin/productos/discos-duros/new">Crear producto</Link>
        </Button>
      </div>
