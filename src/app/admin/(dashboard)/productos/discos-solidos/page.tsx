...
export default async function SsdAdminPage({ searchParams }: Props) {

  const page = (await searchParams).page ? parseInt((await searchParams).page!) : 1;

  const { products } = await getPaginatedProductsWithImages({ page, model: "ssd" });
  ...
  return (
    <section className="flex flex-col container mx-auto px-3 mt-10 lg:mt-20 overflow-hidden">
      ...
      <Title
        ...
        duration={1.2}
      >
        Gestión de discos sólidos
      </Title>

      <div className='mt-5 flex justify-end'>
        <Button>
          <Link href="/admin/productos/discos-solidos/new">Crear producto</Link>
        </Button>
      </div>
