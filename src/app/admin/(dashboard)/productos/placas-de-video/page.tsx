...
export default async function VideoCardAdminPage({ searchParams }: Props) {

  const page = (await searchParams).page ? parseInt((await searchParams).page!) : 1;

  const { products, currentPage, totalPages } = await getPaginatedProductsWithImages({ page, model: "videoCard" });
  ...
  return (
    <section className="flex flex-col container mx-auto px-3 mt-10 lg:mt-20 overflow-hidden">
      ...
      <Title title="Placas de video" subtitle='Administración de base de datos' />

      <div className='flex justify-end mb-5'>
        <Link href="/admin/productos/placas-de-video/new" className="text-slate-200 fon-bold bg-secondary p-2 rounded md:hover:text-white md:hover:bg-secondary/80 transition-colors">Crear producto</Link>
      </div>
...

            {
              products.map((product: IProduct) => (
                <tr key={product.id} className="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
                  ...
                  <td className="px-4">
                    <DeleteProduct productId={product.id} model='videoCard' />
                  </td>
                </tr>
              ))
            }
