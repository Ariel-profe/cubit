...
export default async function CoolerPadsAdminPage({ searchParams }: Props) {
  ...
  const { products } = await getPaginatedProductsWithImages({ page, model: "coolerpad" });
  ...
  return (
    <section className="flex flex-col container mx-auto px-3 mt-10 lg:mt-20 overflow-hidden">
      ...
      <Title
        ...
        duration={1.2}
      >
        Gestión de cooler pads
      </Title>

      <div className='mt-5 flex justify-end'>
        <Button>
          <Link href="/admin/productos/cooler-pads/new">Crear producto</Link>
        </Button>
      </div>
