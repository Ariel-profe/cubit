...
export default async function NetworkCardsAdminPage({ searchParams }: Props) {

  const page = (await searchParams).page ? parseInt((await searchParams).page!) : 1;

  const { products } = await getPaginatedProductsWithImages({ page, model: "networkCard" });  
  ...
  return (
    <section className="flex flex-col container mx-auto px-3 mt-10 lg:mt-20 overflow-hidden">
      ...
      <Title
        ...
        duration={1.2}
      >
        Gestión de placas de red
      </Title>

      <div className='mt-5 flex justify-end'>
        <Button>
          <Link href="/admin/productos/placas-de-red/new">Crear producto</Link>
        </Button>
      </div>
