            ...
            {dimension.width > 0 && (
                <>
                    ...
                    <svg className="absolute top-0 w-full h-[calc(100%+300px)]">
                        <motion.path variants={curve} initial="initial" animate={isExiting ? "exit" : "initial"} fill="#150649" />
                    </svg>
                </>
            )}
        </motion.div>
...
export const DemoOne = ({ children }: { children: ReactNode }) => {

    const [showPreloader, setShowPreloader] = useState(true)

    // View just once in each session navigation
    useEffect(() => {
