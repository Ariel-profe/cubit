...
import { createUpdatePad, deleteProductImage } from "@/actions";
import { ProductImage, Button } from "@/components";
import { IPad, IProductImage } from "@/interfaces";
import { validPadBrands, validPadColorStamp, validPadExtents, validPadMaterial, validPadOpts, validPadSizes } from "@/utils";


interface Props {
    product: Partial<IPad> & { ProductImage?: IProductImage[] } | undefined;
};
...
export const PadForm = ({ product }: Props) => {
    ...
    const { handleSubmit, register, formState: { isValid }, getValues, setValue, watch } = useForm<FormInputs>({
        defaultValues: {
            ...
            colorStamp: product?.colorStamp?.join(", "),
            images: undefined
        }
    });
    ...
    const handleRadioChange = (name: "materials" | "colorStamp", value: string) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        formData.append("extent", productToSave.extent);
        ...
        formData.append("colorStamp", productToSave.colorStamp);
        ...
        const { ok, message } = await createUpdatePad(formData);
        ...
        router.replace("/admin/productos/pads");
    };
            ...
            <div className="grid mb-16 grid-cols-1 sm:grid-cols-2 gap-3 mt-5">
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-col mb-2">
                        ...
                        >
                            <option value="15baeeb6-6820-44f0-96df-e6d676598938">Pads</option>
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                        ...
                        <div className="flex flex-col mb-2 w-full">
                            ...
                            <select
                                className="p-2 border rounded-md bg-slate-200 text-slate-800 capitalize"
                                {...register("extent", { required: true })}
                            >
                                <option value="">[Seleccione]</option>
                                {
                                    validPadExtents.map(opt => (
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                            ...
                            <div className="flex flex-col my-2 w-full">
                                <span>Estampa</span>
                                <div className="flex flex-wrap">
                                    {validPadColorStamp.map((opt) => (
                                        <div
                                            key={opt}
                                            onClick={() => handleRadioChange("colorStamp", opt)}
                                            className={clsx(
                                                "p-2 border cursor-pointer rounded-md mr-2 mb-2 min-w-fit transition-all text-center",
                                                {
                                                    "bg-secondary text-white": (getValues("colorStamp") ?? "").includes(opt),
                                                }
                                            )}
                                        >
                                            <span className="capitalize">{opt}</span>
