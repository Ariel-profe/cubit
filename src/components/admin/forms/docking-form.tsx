...
import { IDocking, IProductImage } from "@/interfaces";

import clsx from "clsx";
import { validDockingBrands, validDockingComp, validDockingConnections, validDockingUsbVersions } from "@/utils";

interface Props {
    product: Partial<IDocking> & { ProductImage?: IProductImage[] } | undefined;
};
...
export const DockingForm = ({ product }: Props) => {
    ...
    const { handleSubmit, register, formState: { isValid }, getValues, setValue, watch } = useForm<FormInputs>({
        defaultValues: {
            ...
            usbVersion: product?.usbVersion?.join(", "),
            images: undefined
        }
    });
    ...
    const handleRadioChange = (name: "compatibility" | "connectionType" | "usbVersion", value: string) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        formData.append("usbVersion", productToSave.usbVersion);
            ...
            <div className="grid mb-16 grid-cols-1 sm:grid-cols-2 gap-3 mt-5">
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-col mb-2">
                        ...
                        >
                            <option value="e86060b8-a80e-44bd-b991-3bba8f619fbe">Docking</option>
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                            ...
                            <div className="flex flex-col my-2 w-full">
                                <span>Conexión</span>
                                <div className="flex flex-wrap">
                                    {validDockingConnections.map((opt) => (
                                        <div
                                            key={opt}
                        ...
                        <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                            <div className="flex flex-col my-2 w-full">
                                <span>Versión USB</span>
                                <div className="flex flex-wrap">
                                    {validDockingUsbVersions.map((opt) => (
                                        <div
                                            key={opt}
                                            onClick={() => handleRadioChange("usbVersion", opt)}
                                            className={clsx(
                                                "p-2 border cursor-pointer rounded-md mr-2 mb-2 min-w-fit transition-all text-center",
                                                {
                                                    "bg-secondary text-white": (getValues("usbVersion") ?? "").includes(opt),
                                                }
                                            )}
                                        >
                                            <span className="capitalize">{opt}</span>
