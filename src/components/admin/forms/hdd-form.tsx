...
import { createUpdateHdd, deleteProductImage } from "@/actions";
import { ProductImage, Button } from "@/components";
import { IHdd, IProductImage } from "@/interfaces";
import { validHddBrands, validHddCapacities, validHddFormats, validHddRpms, validHddThickness } from "@/utils";

interface Props {
    product: Partial<IHdd> & { ProductImage?: IProductImage[] } | undefined;
};
...
export const HddForm = ({ product }: Props) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        formData.append("format", productToSave.format);
        formData.append("capacity", productToSave.capacity);
        formData.append("rpm", productToSave.rpm);
        ...
        const { ok, message } = await createUpdateHdd(formData);
        ...
        router.replace("/admin/productos/discos-duros");
    };
            ...
            <div className="grid mb-16 grid-cols-1 sm:grid-cols-2 gap-3 mt-5">
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-col mb-2">
                        ...
                        >
                            <option value="4af17ee8-12d4-4335-99fd-5ed153c29508">Discos duros</option>
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                        <div className="flex flex-col mb-2 w-full">
                            ...
                            <select
                                className="p-2 border rounded-md bg-slate-200 text-slate-800 capitalize"
                                {...register("rpm", { required: true })}
                            >
                                <option value="">[Seleccione]</option>
                                {
                                    validHddRpms.map(opt => (
                                        <option key={opt} className="capitalize" value={opt}>{opt}</option>
                                    ))
                                }
                            </select>
