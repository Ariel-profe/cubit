...
import { createUpdateNetworkCard, deleteProductImage } from "@/actions";
import { ProductImage, Button } from "@/components";
import { INetworkCard, IProductImage } from "@/interfaces";
import { validNetworkBrands, validNetworkConnectionTypes, validNetworkConnectionUsb, validNetworkSpeeds } from "@/utils";

interface Props {
    product: Partial<INetworkCard> & { ProductImage?: IProductImage[] } | undefined;
};
...
export const NetworkCardForm = ({ product }: Props) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        formData.append("usbConnection", productToSave.usbConnection);
        formData.append("speed", productToSave.speed);
        ...
        const { ok, message } = await createUpdateNetworkCard(formData);
        ...
        router.replace("/admin/productos/placas-de-red");
    };
            ...
            <div className="grid mb-16 grid-cols-1 sm:grid-cols-2 gap-3 mt-5">
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-col mb-2">
                        ...
                        >
                            <option value="4b275049-e228-4b8e-aaac-cbdecaa366b6">Placas de red</option>
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                        ...
                        <div className="flex flex-col mb-2 w-full">
                            ...
                            >
                                <option value="">[Seleccione]</option>
                                {
                                    validNetworkConnectionTypes.map(opt => (
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                        <div className="flex flex-col mb-2 w-full">
                            ...
                            <select
                                className="p-2 border rounded-md bg-slate-200 text-slate-800 capitalize"
                                {...register("usbConnection", { required: true })}
                            >
                                <option value="">[Seleccione]</option>
                                {
                                    validNetworkConnectionUsb.map(opt => (
                                        <option key={opt} className="capitalize" value={opt}>{opt}</option>
                                    ))
                                }
                            </select>
