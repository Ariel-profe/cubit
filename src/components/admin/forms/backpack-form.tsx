...
export const BackpackForm = ({ product }: Props) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        formData.append("color", productToSave.color);
        formData.append("compartments", productToSave.compartments.toString());
        formData.append("lateralPockets", productToSave.lateralPockets.toString());
        formData.append("superiorPockets", productToSave.superiorPockets.toString());
        formData.append("frontPockets", productToSave.frontPockets.toString());
        formData.append("notebookCompartment", productToSave.notebookCompartment);
        ...
        const { ok, message } = await createUpdateBackpack(formData);
        ...
        router.replace("/admin/productos/mochilas");
    };
            ...
            <div className="grid mb-16 grid-cols-1 sm:grid-cols-2 gap-3 mt-5">
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-col mb-2">
                        ...
                        >
                            <option value="6822bdce-9028-49b3-bac6-7b1aa7aecddc">Mochilas</option>
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                        <div className="flex flex-col mb-2 w-full">
                            ...
                            >
                                <option value="">[Seleccione]</option>
                                {
                                    validBackpackCompartments.map(opt => (
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                        <div className="flex flex-col mb-2 w-full">
                            <span>Bolsillos frontales</span>
                            <select
                                className="p-2 border rounded-md bg-slate-200 text-slate-800 capitalize"
                                {...register("frontPockets", { required: true })}
                            >
                                <option value="">[Seleccione]</option>
                                {
                                    validBackpackPockets.map(opt => (
                                        <option key={opt} className="capitalize" value={opt}>{opt}</option>
                                    ))
                                }
                            </select>
                        ...
                        <div className="flex flex-col mb-2 w-full">
                            <span>Bolsillos laterales</span>
                            <select
                                className="p-2 border rounded-md bg-slate-200 text-slate-800 capitalize"
                                {...register("lateralPockets", { required: true })}
                            >
                                <option value="">[Seleccione]</option>
                                {
                                    validBackpackPockets.map(opt => (
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                        <div className="flex flex-col mb-2 w-full">
                            <span>Bolsillos superiores</span>
                            <select
                                className="p-2 border rounded-md bg-slate-200 text-slate-800 capitalize"
                                {...register("superiorPockets", { required: true })}
                            >
                                <option value="">[Seleccione]</option>
                                {
                                    validBackpackPockets.map(opt => (
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                            <div className="flex flex-col my-2 w-full">
                                <span>Compartimiento para notebook</span>
                                <div className="flex flex-wrap gap-2">
                                    {
                                        validBackpackOptions.map(opt => (
                                            <label key={opt} className="cursor-pointer capitalize">
                                                <input
                                                    type="radio"
                                                    value={opt}
                                                    checked={getValues("notebookCompartment") === opt}
                                                    {...register("notebookCompartment", { required: true })}
                            ...
                            <div className="flex flex-col my-2 w-full">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {
                                        validBackpackNotebookSizes.map(opt => (
                                            <label key={opt} className="cursor-pointer capitalize">
                                                <input
                                                    type="radio"
                                                    value={opt}
                                                    checked={getValues("notebookSize") === opt}
