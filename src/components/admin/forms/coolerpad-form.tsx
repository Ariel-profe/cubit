...
import { createUpdateCoolerpad, deleteProductImage } from "@/actions";
import { ProductImage, Button } from "@/components";
import { ICoolerpad, IProductImage } from "@/interfaces";
import { validCoolerBrands, validCoolerFan, validCoolerHeigthLevels, validCoolerOpts, validCoolerUsb } from "@/utils";

interface Props {
    product: Partial<ICoolerpad> & { ProductImage?: IProductImage[] } | undefined;
};
...
export const CoolerpadForm = ({ product }: Props) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        formData.append("heightLevel", productToSave.heightLevel.toString());
        formData.append("fan", productToSave.fan.toString());
        formData.append("usbPorts", productToSave.usbPorts.toString());
        ...
        const { ok, message } = await createUpdateCoolerpad(formData);
        ...
        router.replace("/admin/productos/cooler-pads");
    };
            ...
            <div className="grid mb-16 grid-cols-1 sm:grid-cols-2 gap-3 mt-5">
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-col mb-2">
                        ...
                        >
                            <option value="5ab58152-b266-4e4a-9069-f9aa20eb5041">Cooler pads</option>
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                            <div className="flex flex-col my-2 w-full">
                                <span>Nivel de altura</span>
                                <div className="flex flex-wrap gap-2">
                                    {
                                        validCoolerHeigthLevels.map(opt => (
                                            <label key={opt} className="cursor-pointer capitalize">
                                                <input
                                                    type="radio"
                                                    value={opt}
                                                    checked={String(getValues("heightLevel")) === String(opt)}
                                                    {...register("heightLevel", { required: true })}
                            ...
                            <div className="flex flex-col my-2 w-full">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {
                                        validCoolerFan.map(opt => (
                                            <label key={opt} className="cursor-pointer capitalize">
                                                <input
                                                    type="radio"
                                                    value={opt}
                                                    checked={String(getValues("fan")) === String(opt)}
                                                    {...register("fan", { required: true })}
                                                    className="cursor-pointer mr-1"
                                                />
                                                {opt}
                                            </label>
                        ...
                        <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
                            <div className="flex flex-col my-2 w-full">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {
                                        validCoolerUsb.map(opt => (
                                            <label key={opt} className="cursor-pointer capitalize">
                                                <input
                                                    type="radio"
                                                    value={opt}
                                                    checked={String(getValues("usbPorts")) === String(opt)}
                                                    {...register("usbPorts", { required: true })}
                                                    className="cursor-pointer mr-1"
                                                />
                                                {opt}
                                            </label>
                            ...
                            <div className="flex flex-col my-2 w-full">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {
                                        validCoolerOpts.map(opt => (
                                            <label key={opt} className="cursor-pointer capitalize">
                                                <input
                                                    type="radio"
                                                    value={opt}
                                                    checked={getValues("rgbLighting") === opt}
